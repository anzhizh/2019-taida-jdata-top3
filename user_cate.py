...
def get_accumulate_user_feat_v1(start_date, end_date):
    dump_path = './cache/user_feat_accumulate_F11_7_%s_%s.pkl' % (start_date, end_date)
...
def get_actions_product_cart(start_date, end_date):
    dump_path = './cache/all_action_product_cart__F11_7_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
...
def get_accumulate_user_feat_v1_cart(start_date, end_date):
    dump_path = './cache/user_feat_accumulate_F11_7_%s_%s.pkl' % (start_date, end_date)
...
def get_stat_feat_v1(start_date, end_date):
    dump_path = './cache/stat_feat_accumulate_F11_7_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
...
def get_cross_feat_v1(start_date, end_date):
    dump_path = './cache/cross_feat_F11_7_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
        actions = pd.read_pickle(dump_path)
    else:
...
def get_user_feat15_v1(start_date, end_date):
    dump_path = './cache/user_feat15_v1_F11_7_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
        actions = pd.read_pickle(dump_path)
...
def get_labels_v1(start_date, end_date):
    dump_path = './cache/labels_F11_7_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
        actions = pd.read_pickle(dump_path)
    else:
        actions = get_actions_product(start_date, end_date)
...
def make_train_set_F11_7(train_start_date, train_end_date, test_start_date, test_end_date, days=30):
    dump_path = './cache/train_set_F11_7_%s_%s_%s_%s.pkl' % (
    ...
    if os.path.exists(dump_path):
        ...
        f11_actions = f11_actions[['user_id', 'cate']].drop_duplicates()

        # 标签
        labels = get_labels_v1(test_start_date, test_end_date)
        ...
        start_days = start_days.strftime('%Y-%m-%d')
        actions_cart = get_accumulate_user_feat_v1_cart(start_days, train_end_date)

        # act_5
        # act5_feat = pd.read_csv('./cache_final/train_lastday_act5_stat.csv')
        ...
        actions = pd.merge(actions, cate_feat11, how='left', on='cate')                # 用于concat
        print('actions1 finished')
        """
        F11
        """
...
def make_test_set_F11_7(train_start_date, train_end_date):
    dump_path = './cache/test_set_F11_7_%s_%s.pkl' % (train_start_date, train_end_date)
    if os.path.exists(dump_path):
        actions = pd.read_pickle(dump_path)
    else:
        # 索引
        ...
        # act_5
        # act5_feat = pd.read_csv('./cache_final/test_lastday_act5_stat.csv')
        act5_feat = get_last1day_cart_fearture(start_days, train_end_date, 1)
        act5_feat = act5_feat.groupby(['user_id', 'cate'], as_index=False).sum()
        del act5_feat['shop_id']
...
def lgb_train_F11_7(X_train1, y_train1, X_test1, sub_user_index):
    # 提交结果
    ...
    sub.to_csv('./res/sub_F11_7.csv', index=False, index_label=False)